...
function inInTimeRange(historyReadDetails, dataValue) {
    if (historyReadDetails.startTime &&
        !node_opcua_date_time_1.isMinDate(historyReadDetails.startTime)
        && dataValue.sourceTimestamp < historyReadDetails.startTime) {
        return false;
    }
    if (historyReadDetails.endTime
        && !node_opcua_date_time_1.isMinDate(historyReadDetails.endTime)
...
class VariableHistorian {
    ...
    push(newDataValue) {
        ...
        if (this._timeline.length > this._maxOnlineValues) {
            node_opcua_assert_1.assert(_.isNumber(this._maxOnlineValues) && this._maxOnlineValues > 0);
...
function _update_startOfArchive(newDate) {
    ...
    node.$historicalDataConfiguration.startOfArchive.setValueFromSource({
        dataType: node_opcua_variant_1.DataType.DateTime, value: newDate
    });
}
function _update_startOfOnlineArchive(newDate) {
    ...
    if (startOfArchiveDataValue.statusCode !== node_opcua_status_code_1.StatusCodes.Good ||
...
ua_variable_1.UAVariable.prototype._update_startOfOnlineArchive = _update_startOfOnlineArchive;
ua_variable_1.UAVariable.prototype._update_startOfArchive = _update_startOfArchive;
function _historyPush(newDataValue) {
    const node = this;
    if (!node.varHistorian) {
        throw new Error("this variable has no HistoricalDataConfiguration");
    }
    node_opcua_assert_1.assert(node.hasOwnProperty("historizing"), "expecting a historizing attribute on node");
    if (!node.historizing) {
        return; //
    }
    node_opcua_assert_1.assert(node.historizing === true);
...
function _historyReadRaw(context, historyReadRawModifiedDetails, indexRange, dataEncoding, continuationPoint, callback) {
    const node = this;
    node_opcua_assert_1.assert(historyReadRawModifiedDetails instanceof node_opcua_service_history_1.ReadRawModifiedDetails);
    ...
    if (node_opcua_date_time_1.isMinDate(historyReadRawModifiedDetails.endTime)) {
        // end time is not specified
        maxNumberToExtract = historyReadRawModifiedDetails.numValuesPerNode;
        if (node_opcua_date_time_1.isMinDate(historyReadRawModifiedDetails.startTime)) {
    ...
    else if (node_opcua_date_time_1.isMinDate(historyReadRawModifiedDetails.startTime)) {
    ...
    node._historyReadRawAsync(historyReadRawModifiedDetails, maxNumberToExtract, isReversed, reverseDataValue, (err, dataValues) => {
        if (err || !dataValues) {
            return callback(err);
        }
        ...
        if (historyReadRawModifiedDetails.numValuesPerNode >= 1) {
            if (dataValues.length === 0) {
                ...
                if (remaining.length > 0 && !node_opcua_date_time_1.isMinDate(historyReadRawModifiedDetails.endTime)) {
...
function AddressSpace_installHistoricalDataNode(node, options) {
    source_1.AddressSpace.historizerFactory = source_1.AddressSpace.historizerFactory || {
    ...
    node.varHistorian = options.historian || source_1.AddressSpace.historizerFactory.create(node, options);
